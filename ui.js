  ...
  return `
    <div class="task-card-mini priority-${task.priority}" data-id="${task.id}" data-action="open">
      ...
      <div class="mini-body">
        ...
        ${lastStatus ? `<div class="mini-status-line">🔵 ${escapeHtml(lastStatus.text)} <span class="mini-status-date">· ${formatShortDate(toDateKey(new Date(lastStatus.date)))}</span></div>` : ''}
      </div>
    </div>`;
}
