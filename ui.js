...
function renderTaskCard(task, cat) {
  ...
  const lastStatus = TaskManager.lastStatusUpdate(task);
  ...
  return `
    <div class="task-card priority-${task.priority} ${isDone ? 'is-done' : ''}" data-id="${task.id}">
      ...
      <div class="task-body" data-action="open">
        ...
        ${lastStatus ? `<div class="task-status-line">🔵 ${escapeHtml(lastStatus.text)} <span class="task-status-date">· ${formatTimestamp(lastStatus.date)}</span></div>` : ''}
      </div>
  ...
  async open(taskId = null) {
    ...
    if (task) {
      ...
      const submitStatus = async () => {
        ...
        if (!text) return;
        await TaskManager.addStatusUpdate(task.id, text);
        statusInput.value = '';
        const fresh = await TaskManager.getById(task.id);
