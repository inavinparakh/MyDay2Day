...
function renderTaskCard(task, cat) {
  ...
  meta.push(`Started ${formatShortDate(task.originalDate)}`);
  const statusLog = task.statusLog || [];
  const historyHtml = statusLog.length
    ? [...statusLog].reverse().map(e => `<div class="task-status-line">🔵 ${escapeHtml(e.text)} <span class="task-status-date">· ${formatTimestamp(e.date)}</span></div>`).join('')
    : '';
...
/* --- Compact tiles for the Today 4-column grid --- */
function renderTaskTile(task, cat, isDone = false) {
  const meta = [];
  if (task.dueTime) meta.push(`<span class="tile-time">${formatTime12(task.dueTime)}</span>`);
  if (task.status === 'carried') meta.push(`<span class="tile-carried">🔄</span>`);
  if (cat) meta.push(`<span class="tile-cat-dot" style="background:${cat.color}"></span>`);

  return `
    <div class="task-tile priority-${task.priority} ${isDone ? 'is-done' : ''}" data-id="${task.id}">
      <button class="tile-check ${isDone ? 'checked' : ''}" data-action="toggle" aria-label="Mark complete">${isDone ? '✓' : ''}</button>
      <div class="tile-title">${escapeHtml(task.title)}</div>
      <div class="tile-meta">${meta.join('')}</div>
    </div>`;
}

function bindTileEvents(container) {
  container.querySelectorAll('.task-tile').forEach(tile => {
    const id = tile.dataset.id;
    const checkBtn = tile.querySelector('[data-action="toggle"]');
    checkBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      await TaskManager.complete(id);
      App.refreshCurrentView();
    });
    tile.addEventListener('click', () => TaskModal.open(id));
  });
}
...
const TaskModal = {
  ...
  _renderStatusLog(task) {
    const log = task.statusLog || [];
    if (!log.length) return `<div class="status-log-empty">No status updates yet.</div>`;
    return [...log].reverse().map(entry => `
      <div class="status-log-row">
        <span class="status-log-text">${escapeHtml(entry.text)}</span>
        <span class="status-log-date">${formatTimestamp(entry.date)}</span>
      </div>`).join('');
  },
  ...
  async open(taskId = null) {
        ...
        <div class="modal-sheet">
          <div class="modal-header">
            <div>
              ...
              ${task ? `<div class="modal-subtitle">Started ${formatShortDate(task.originalDate)}</div>` : ''}
            </div>
            <button class="icon-btn" id="modal-close" aria-label="Close">✕</button>
          </div>
          ${task ? `
          <div class="status-log-block">
            <div class="status-log-title">Status updates</div>
            <div class="status-log-list" id="status-log-list">${this._renderStatusLog(task)}</div>
            <div class="status-log-add">
              <input type="text" id="status-log-input" maxlength="120" placeholder="e.g. Documents submitted, awaiting approval">
              <div class="status-log-datetime">
                <input type="date" id="status-log-date" value="${todayKey()}">
                <input type="time" id="status-log-time" value="${nowHHMM()}">
                <button type="button" class="btn btn-secondary" id="status-log-add-btn">Add</button>
              </div>
              <div class="status-log-hint">This date &amp; time becomes the task's next follow-up — the task will show on that date, with a reminder at that time.</div>
            </div>
          </div>` : ''}
    ...
    if (task) {
      ...
      const addStatusBtn = document.getElementById('status-log-add-btn');
      const statusInput = document.getElementById('status-log-input');
      const statusDateInput = document.getElementById('status-log-date');
      const statusTimeInput = document.getElementById('status-log-time');
      const submitStatus = async () => {
        const text = statusInput.value.trim();
        if (!text) return;
        const dateVal = statusDateInput.value || todayKey();
        const timeVal = statusTimeInput.value || nowHHMM();
        const [y, m, d] = dateVal.split('-').map(Number);
        const [hh, mm] = timeVal.split(':').map(Number);
        const scheduledAt = new Date(y, m - 1, d, hh, mm).getTime();

        const fresh = await TaskManager.addStatusUpdate(task.id, text, scheduledAt);
        statusInput.value = '';
        statusDateInput.value = todayKey();
        statusTimeInput.value = nowHHMM();
        document.getElementById('status-log-list').innerHTML = this._renderStatusLog(fresh);
        // Keep the open form's own due-date/time/reminder fields in sync with
        // the reschedule so Save doesn't overwrite it with the old values.
        overlay.querySelector('input[name="dueDate"]').value = fresh.dueDate;
        overlay.querySelector('input[name="dueTime"]').value = fresh.dueTime || '';
        overlay.querySelector('select[name="reminder"]').value = fresh.reminder != null ? String(fresh.reminder) : '';
        App.refreshCurrentView();
      };
      addStatusBtn.addEventListener('click', submitStatus);
      statusInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') { e.preventDefault(); submitStatus(); }
      });
    }
