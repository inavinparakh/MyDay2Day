...
const App = {
  ...
  async renderToday() {
    ...
    // Quadrant 1 — Today's Schedule: tasks genuinely due today (not carried
    // over from an earlier day) — time-scheduled first, then anytime.
    ...
    // Quadrant 2 — Important: today's open High Priority tasks (from either
    // bucket below). It's the same underlying task record everywhere, so a
    // status update or completion made from any one place is reflected
    // wherever else that task appears.
    const important = tasks.filter(t => t.priority === 'high' && t.status !== 'completed' && t.status !== 'cancelled')
      .sort((a, b) => (a.dueTime || '').localeCompare(b.dueTime || ''));

    // Quadrant 3 — Pending & Carry Forward: ONLY tasks carried over from an
    // earlier day (status 'carried'). Today's fresh tasks live in Quadrant 1
    // instead, so nothing appears in both — one entry per task, ever.
    const carriedOnly = tasks.filter(t => t.status === 'carried')
      .sort((a, b) => (a.dueTime || '').localeCompare(b.dueTime || ''));
    ...
    this._fillQuad('today-pending-list', 'count-pending', carriedOnly, byCat,
      'No carried-forward tasks — everything from before is either done or cancelled.');
