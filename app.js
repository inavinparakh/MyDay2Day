...
const App = {
  ...
  async renderToday() {
    ...
    // Quadrant 1 — Today's Schedule: everything open today, time-scheduled
    // items first (in time order), then anytime items.
    const scheduledOpen = tasks.filter(t => t.dueTime && t.status !== 'completed' && t.status !== 'cancelled')
      .sort((a, b) => a.dueTime.localeCompare(b.dueTime));
    const anytimeOpen = tasks.filter(t => !t.dueTime && (t.status === 'pending' || t.status === 'carried'));
    const scheduleItems = [...scheduledOpen, ...anytimeOpen];

    // Quadrant 2 — Important: today's open High Priority tasks. A task can
    // legitimately also appear in Schedule / Pending & Carry Forward — it's
    // the same underlying task record everywhere, so a status update or
    // completion made from any one place is reflected in all of them.
    const important = tasks.filter(t => t.priority === 'high' && t.status !== 'completed' && t.status !== 'cancelled')
      .sort((a, b) => (a.dueTime || '').localeCompare(b.dueTime || ''));

    // Quadrant 3 — Pending & Carry Forward: the full open worklist for
    // today (pending + carried-forward). Carry-forward never creates a
    // second copy of a task — TaskManager.runCarryForward() moves the same
    // record's dueDate forward and marks it 'carried', so it stays one entry.
    const pendingCarried = tasks.filter(t => t.status === 'pending' || t.status === 'carried')
      .sort((a, b) => (a.dueTime || '').localeCompare(b.dueTime || ''));

    // Quadrant 4 — Completed: today's completed tasks.
    const done = tasks.filter(t => t.status === 'completed');

    this._fillQuad('today-schedule-list', 'count-schedule', scheduleItems, byCat,
      'Nothing planned for today yet. Tap + to add your first task.');
    this._fillQuad('today-important-list', 'count-important', important, byCat,
      'No high-priority tasks today.');
    this._fillQuad('today-pending-list', 'count-pending', pendingCarried, byCat,
      'All clear 🎉 Nothing pending or carried forward.');
    this._fillCompletedQuad('today-completed-list', 'count-completed', done, byCat);
  },

  _fillQuad(listId, countId, items, byCat, emptyMessage) {
    ...
    container.innerHTML = `<div class="task-mini-list">${items.map(t => renderCompactTaskCard(t, byCat(t))).join('')}</div>`;
    bindCompactTaskEvents(container);
  },

  _fillCompletedQuad(listId, countId, items, byCat) {
    const container = document.getElementById(listId);
    document.getElementById(countId).textContent = items.length;
    if (!items.length) {
    ...
    container.innerHTML = `<div class="task-mini-list">${items.map(t => renderCompletedMiniCard(t, byCat(t))).join('')}</div>`;
    bindCompactTaskEvents(container);
  },
