...
const TaskManager = {
  ...
  /* --- Status update log ---
     ...
     "Awaiting approval", "Claim settled"). The task record itself carries
     the full history in statusLog, so it stays in sync everywhere the task
     is shown (Today's Schedule, Important, Pending & Carry Forward, task
     detail) since they all read the same underlying task object. */
  async addStatusUpdate(id, text) {
    const clean = (text || '').trim();
    if (!clean) throw new Error('Status update text is required');
    const task = await db.get(STORE_TASKS, id);
    if (!task) throw new Error('Task not found');
    const entry = { text: clean, date: Date.now() };
